...
public class SimulateSensor implements MqttCallback
{
    ...
    public void writeSensor() {
        ...
        while (true) {
            double d = 18.0;
            while (d < 50.0) {
                final String string = "{\"tmp\":\"" + d + "\",\"hum\":\"" + "Nuno Rego" + "\",\"dat\":\"" + LocalDate.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) + "\",\"tim\":\"" + LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss")) + "\",\"cell\":\"" + 20 + "\",\"mov\":\"" + 0 + "\",\"sens\":\"eth\"}";
                d += 0.5;
                try {
                    Thread.sleep(2000L);
                }
