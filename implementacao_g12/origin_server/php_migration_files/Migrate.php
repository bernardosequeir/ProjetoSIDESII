<?php
   ...
   */
   function migrateLogs($conn1, $conn2) {
      ...
      $last_id = 0;
      $last_id_migrated = $conn2->query("SELECT * FROM g12_log ORDER BY idLog DESC LIMIT 1");
      
      if ($last_id_migrated->num_rows > 0) {
         // output data of each row
         while($row = $last_id_migrated->fetch_assoc()) {
             echo "<i>Last Log migrated was: " . $row["idLog"] . "</i></br>";
             $last_id = $row["idLog"];
         }
      } else {
         echo "<i>This is the first migration </i></br>";
      }

      $new_logs = $conn1->query("SELECT * FROM log WHERE log.idLog > " ./*getOriginalID()*/ $last_id . ";");
